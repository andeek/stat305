...
\vspace{.2in}
\begin{df}
The \emph{$p^{th}$ percentile} of a dataset is a number greater than $p$\% of the data and less than the rest.
\end{df}
...
$$Q(p) = Q\left(\frac{i-.5}{n}\right) = x_i$$ (the $i$th smallest data point will be called th $\frac{i-.5}{n}$ quantile)
...
\begin{df}
A \emph{Q-Q plot} for two data sets with respective quantile functions $Q_1$ and $Q_2$ is a plot of ordered pais $(Q_1(p), Q_2(p))$ for appropriate values of $p$. When two data sets of size $n$ are involved, the values of $p$ used to make the plot will be $\frac{i-.5}{n}$ for $i = 1, \dots, n$. 
\end{df}
...
Make a Q-Q plot for the following small artificial datasets.
